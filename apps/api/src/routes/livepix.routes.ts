...
export async function livePixRoutes(fastify: FastifyInstance) {
  fastify.get('/livepix/oauth/callback', async (request, reply) => {
    ...
    try {
      const result = await handle_livepix_oauth_callback({
        code: query.code,
        state: query.state,
        log: request.log,
      })

      const redirect = new URL(`/guild/${result.guildId}/support`, CONFIG.web.url)
  ...
  fastify.post('/livepix/webhook', async (request, reply) => {
    ...
    if (clientId && CONFIG.livePix.clientId && clientId !== CONFIG.livePix.clientId) {
    ...
    let event_id: string
    try {
      const created = await prisma.livePixWebhookEvent.create({
        data: {
          dedupeKey,
          clientId,
          event: payload.event,
    ...
    if (resourceType !== 'payment' || !reference) {
      await prisma.livePixWebhookEvent.update({
        where: { id: event_id },
        data: {
          status: 'IGNORED',
    ...
    try {
      const fulfillment = await verify_and_fulfill_support_payment(
