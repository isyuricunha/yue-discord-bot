...
export class GroqClient {
	...
	async create_completion(
		input: groq_completion_input
	): Promise<groq_completion_result> {
	...
	private async request_with_key(
		key_index: number,
		request: groq_chat_completion_request
	): Promise<string> {
		...
		try {
			...
			const retry_after_seconds = parse_retry_after_seconds(
			...
			if (!res.ok) {
				...
				const msg =
					body && typeof body === "object" && "error" in body
						? String(
								((body as Record<string, any>).error?.message ?? "") as string
							)
						: `Groq API returned ${res.status}`;

				throw new GroqApiError(
					msg || `Groq API returned ${res.status}`,
					res.status,
					body,
					retry_after_seconds
				);
