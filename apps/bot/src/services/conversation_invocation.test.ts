...
import { build_user_prompt_from_invocation, contains_yue_keyword, remove_bot_mention } from './conversation_invocation'

test('conversation_invocation: matches yue keyword case-insensitive as word', () => {
...
test('conversation_invocation: removes bot mention markup', () => {
  assert.equal(remove_bot_mention('<@123> hello', '123'), 'hello')
  assert.equal(remove_bot_mention('<@!123> hello', '123'), 'hello')
  assert.equal(remove_bot_mention('hi <@123> there', '123'), 'hi there')
})

test('conversation_invocation: builds user prompt from mention or yue keyword', () => {
  assert.equal(
