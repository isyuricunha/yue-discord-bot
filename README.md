...
In addition to the standard image tags (for example `latest` and `v<version>`), CI also publishes **SlimToolkit-minified** variants:

- **`latest-slim`** and **`v<version>-slim`** (multi-arch manifests)
- **`v<version>-slim-amd64`** and **`v<version>-slim-arm64`** (arch-specific tags)

## Versioning and releases

This project uses **automated semantic versioning** on the `main` branch.

- Releases are generated by **semantic-release** based on **Conventional Commits**.
- Git tags follow the format **`v${version}`** (for example `v1.4.0`).
- Docker images are published only when a new release is created.
...
- **`DOCKERHUB_USERNAME`**: required to push images to Docker Hub.
- **`DOCKERHUB_TOKEN`**: required to push images to Docker Hub.
...
The release type is derived from the commit messages:

- `feat(scope): add new feature` -> **minor**
- `fix(scope): correct bug` -> **patch**
- `perf(scope): improve performance` -> **patch**
- `docs(scope): update docs` -> **no release** (unless it includes BREAKING CHANGE)

Breaking changes:

- Add `!` after the type (for example `feat!: ...`), or
- Add a footer `BREAKING CHANGE: ...`

### Docker tags

When a new release is published (for example `v1.4.0`), Docker images are pushed with:
